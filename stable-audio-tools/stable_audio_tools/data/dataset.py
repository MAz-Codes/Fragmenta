...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True):
    ...
    if dataset_type == "audio_dir":
        ...
        for audio_dir_config in audio_dir_configs:
            ...
            # load custom metadata function by module path, if provided
            module_path = audio_dir_config.get("custom_metadata_module", None)
            if module_path is not None:
                metadata_module = importlib.import_module(module_path)
        ...
        for pre_encoded_dir_config in pre_encoded_dir_configs:
            ...
            # load custom metadata function if a module path was given
            module_path = pre_encoded_dir_config.get("custom_metadata_module", None)
            if module_path is not None:
                metadata_module = importlib.import_module(module_path)
                custom_metadata_fn = metadata_module.get_custom_metadata
            else:
        ...
        for wds_config in dataset_config["datasets"]:
            ...
            if custom_metadata_module_path is not None:
                ...
                sys.modules["metadata_module"] = metadata_module
